...
//               {settings?.footerTagline || "Click N Troway — Making waste management easy for everyone in Freetown."}
//             </p>

//             {/* USSD */}
//             <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-2xl px-4 py-3 mb-6">
//               <span className="text-sage font-body text-xs">No smartphone? Dial</span>
//               <span className="font-heading font-black text-amber text-base">{ussdCode}</span>
//             </div>

//             {/* Social links */}
//             <div className="flex gap-3">
//               {[
//                 { label: "Facebook", href: settings?.socialLinks?.facebook || "#", icon: "f" },
//                 { label: "Instagram", href: settings?.socialLinks?.instagram || "#", icon: "ig" },
//                 { label: "X", href: settings?.socialLinks?.twitter || "#", icon: "x" },
//                 { label: "LinkedIn", href: settings?.socialLinks?.linkedin || "#", icon: "in" },
//               ].map((s) => (
//                 <a
//                   key={s.label}
//                   href={s.href}
...
//             <div key={group}>
//               <h4 className="font-heading font-semibold text-white text-sm mb-4">{group}</h4>
...
//             Copyright © {new Date().getFullYear()} | DortiBox — Freetown Waste Transformers SL Limited
//           </p>
...
export function Footer({ settings }: FooterProps) {
  const ussdCode = settings?.ussdCode || "*715*380#";
  const tagline = settings?.footerTagline || "Click N Troway — Waste Management Made Easy";

  const socials = [
    { label: "Facebook", href: settings?.socialLinks?.facebook || "#", abbr: "f" },
    { label: "Instagram", href: settings?.socialLinks?.instagram || "#", abbr: "ig" },
    { label: "X", href: settings?.socialLinks?.twitter || "#", abbr: "x" },
    { label: "LinkedIn", href: settings?.socialLinks?.linkedin || "#", abbr: "in" },
  ];
